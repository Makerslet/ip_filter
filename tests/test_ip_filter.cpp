...
TEST(FILTER, SIMPLE_FILTER)
{
    ...
    // Act
    std::vector<ip_addr<4>> filtered = Filter::filter<4>(adresses, {0, 13});
...
TEST(FILTER, AND_FILTER)
{
    ...
    // Act
    std::vector<ip_addr<4>> filtered = Filter::filter_and<4>(adresses, {{0, 13},{1, 14}});
...
TEST(FILTER, ANY_FILTER)
{
    ...
    // Act
    std::list<ip_addr<4>> filtered = Filter::filter_any<4>(adresses, 10);
